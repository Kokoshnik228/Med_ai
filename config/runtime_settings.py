...
RUNTIME = {
    ...
    "ollama": {
        ...
        "model": "llama3.1:8b",
        "max_tokens": 220,     # сколько токенов генерировать (длина ответа)
        "timeout_s": 30,       # таймаут HTTP запроса к LLM (секунды)
        "temperature": 0.1,
        "top_p": 0.9,
    ...
    # ── Ретраивл ────────────────────────────────────────────────────
    "reranker": {
        "enabled": False,                         # <— тумблер
        "model": "BAAI/bge-reranker-v2-m3",       # cross-encoder модель
        "top_n": 50,                              # сколько кандидатов переоценивать
        "device": "cpu"                           # cpu | cuda
    },
